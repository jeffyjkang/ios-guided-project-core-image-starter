...
//            let scale: CGFloat = 0.5
            // create property on scale to blur appropriately, otherwise blur will not transfer over exactly to output image
...
//        print(filter.attributes)
	...
	@IBAction func savePhotoButtonPressed(_ sender: UIButton) {
        ...
        guard let originalImage = originalImage?.flattened,
              let originalCIImage = CIImage(image: originalImage) else { return }
        
        guard let processedImage = image(byFiltering: originalCIImage) else { return }
